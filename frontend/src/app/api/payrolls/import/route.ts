...
export async function POST(req: Request) {
    try {
        ...
        console.log("Forwarding import request to backend:", PAYROLLS_IMPORT_URL);
        const res = await fetch(PAYROLLS_IMPORT_URL, {
            method: "POST",
            headers,
            body: uploadFormData,
        });
        ...
        if (!res.ok) {
            ...
            console.error("External API (POST import payrolls) error:", res.status, errorData);
            return NextResponse.json(
        ...
        console.error("POST /api/payrolls/import exception:", err);
