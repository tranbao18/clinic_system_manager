...
export async function POST(req: Request) {
    try {
        ...
        if (!res.ok) {
            ...
            console.error("❌ External API (POST vnpay/create) error:", {
                status: res.status,
                statusText: res.statusText,
                url: `${BACKEND_URL}/api/payments/vnpay/create`,
                error: errorData,
                hasAuth: !!authHeaders.Authorization,
            ...
            return NextResponse.json(
                {
                    error: errorData.error || errorData.message || "Không thể tạo URL thanh toán VNPay",
        ...
        console.error("POST /api/payments/vnpay/create exception:", err);
        return NextResponse.json(
